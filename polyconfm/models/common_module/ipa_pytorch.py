...
def rot2trans(rigids, input_coord, key_point_index):
    """Convert rigid rotations into cumulative translation trajectory.

    Args:
        rigids: Rigid object containing per-residue rotation frames.
        input_coord: Local coordinates used to derive stepwise offsets.
        key_point_index: Indices selecting anchor points for offset computation.

    Returns:
        Updated rigid object whose translation component is reassigned.
    """
    frames = rigids
...
def permute_final_dims(tensor: torch.Tensor, inds: List[int]):
    """Permute only trailing dimensions while preserving leading axes.

    Args:
        tensor: Input tensor with arbitrary leading dimensions.
        inds: Permutation order applied to final len(inds) dimensions.

    Returns:
        Tensor with unchanged leading dimensions and permuted trailing ones.
    """
...
def flatten_final_dims(t: torch.Tensor, no_dims: int):
    """Flatten the final dimensions of a tensor into one feature axis.

    Args:
        t: Input tensor.
        no_dims: Number of trailing dimensions to collapse.

    Returns:
        Reshaped tensor where the last no_dims are merged.
    """
    return t.reshape(t.shape[:-no_dims] + (-1,))


def ipa_point_weights_init_(weights):
    """Initialize IPA point weights to unit value after softplus.

    Args:
        weights: Learnable head-weight tensor to initialize in place.
...
def _prod(nums):
    """Compute multiplicative product of values in an iterable.

    Args:
        nums: Iterable of numeric values.

    Returns:
        Scalar product of all provided values.
    """
    out = 1
    for n in nums:
        out = out * n
...
def _calculate_fan(linear_weight_shape, fan="fan_in"):
    """Compute selected fan statistic from linear weight shape.

    Args:
        linear_weight_shape: Weight shape as (fan_out, fan_in).
        fan: One of fan_in, fan_out, or fan_avg.

    Returns:
        Fan value used to scale parameter initialization.
    """
...
def trunc_normal_init_(weights, scale=1.0, fan="fan_in"):
    """Initialize weights with fan-scaled truncated normal samples.

    Args:
        weights: Parameter tensor to initialize in place.
        scale: Base variance scaling factor.
        fan: Fan mode, one of fan_in, fan_out, or fan_avg.

    Returns:
        None. The weights tensor is modified in place.
    """
    shape = weights.shape
...
def lecun_normal_init_(weights):
    """Apply LeCun-style initialization using truncated normal.

    Args:
        weights: Parameter tensor to initialize.

    Returns:
        None. Initialization is applied in place.
    """
    trunc_normal_init_(weights, scale=1.0)


def he_normal_init_(weights):
    """Apply He-style initialization using truncated normal.
...
def glorot_uniform_init_(weights):
    """Apply Xavier or Glorot uniform initialization.
...
def gating_init_(weights):
    """Initialize gating projection weights to zero.
...
def normal_init_(weights):
    """Apply Kaiming normal initialization for linear layers.
...
def compute_angles(ca_pos, pts):
    """Compute neighbor-frame angles between CA vectors and IPA points.

    Args:
        ca_pos: CA coordinates with shape [B, N, 3].
        pts: IPA points with shape [B, N, H, P, 3].

    Returns:
        Angle tensor with shape [B, N, N, H, P].
    """
...
class StructureModuleTransition(nn.Module):
    ...
    def forward(self, s):
        """Apply residual transition MLP with layer normalization."""
        s_initial = s
        s = self.linear_1(s)
        s = self.relu(s)
        s = self.linear_2(s)
...
class EdgeTransition(nn.Module):
    ...
    def forward(self, node_embed, edge_embed):
        """Update pair embeddings using projected node context features."""
...
class TorsionAngles(nn.Module):
    ...
    def forward(self, s):
        """Predict torsion angle vectors and their normalized directions."""
        s_initial = s
        s = self.linear_1(s)
        s = self.relu(s)
        s = self.linear_2(s)
...
class ScoreLayer(nn.Module):
    ...
    def forward(self, s):
        """Apply residual MLP head for score prediction."""
        s_initial = s
        s = self.linear_1(s)
        s = self.relu(s)
        s = self.linear_2(s)
...
class IpaScore(nn.Module):
    ...
    def forward(self, init_node_embed, edge_embed, node_mask, edge_mask, rigids_t, t, input_coord=None, key_point_index=None):
        """Predict rotational and translational scores from IPA-refined rigids."""
