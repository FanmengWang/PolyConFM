...
@register_model("polyconfm_property")
class PolyConFMPropertyModel(BaseUnicoreModel):
    @staticmethod
    def add_args(parser):
    ...
    def forward_mae_encoder(self, x, mask, padding_mask, class_embedding, buffer_size):
        """Encode MAR inputs with class conditioning and buffer tokens."""
        x = self.mar_z_proj(x)
    ...
    def forward(
        ...
        **kwargs
        ...
        """Run property forward pass and output regression logits for a named head."""
        ...
        mean_mar_x = masked_mean_pooling(x=mar_x, padding_mask=mar_padding_mask)
        logits = self.classification_heads[classification_head_name](mean_mar_x)
        return (logits, )
    ...
    def register_classification_head(
        ...
        self.classification_heads[name] = ClassificationHead(
            input_dim=self.args.mar_encoder_embed_dim,
            inner_dim=inner_dim or self.args.mar_encoder_embed_dim,
...
# Mean pooling, considering the padding mask to only average over valid tokens
def masked_mean_pooling(x, padding_mask):  
    """Compute mean pooling over valid positions indicated by the padding mask."""
...
class RotaryPositionEmbedding:  
    def __init__(self, d_model, base=10000):  
        """Initialize rotary positional embedding helper."""
        self.d_model = d_model  
        self.base = base  

    def rotate(self, x):  
        """Apply rotary-style position mixing to an input sequence tensor."""
...
class ClassificationHead(nn.Module):
    ...
    def forward(self, features, **kwargs):
        """Project pooled features into task-specific regression or classification outputs."""
        x = features
        x = self.dense(x)
...
class NonLinearHead(nn.Module):
    ...
    def forward(self, x):
        """Apply a two-layer MLP projection with activation."""
        x = self.linear1(x)
        x = self.activation_fn(x)
        x = self.linear2(x)
        return x
...
def gaussian(x, mean, std):
    """Evaluate Gaussian basis values for distance feature expansion."""
    pi = 3.14159
    a = (2 * pi) ** 0.5
...
class GaussianLayer(nn.Module):
    ...
    def forward(self, x, edge_type):
        """Encode pair distances into Gaussian radial basis features."""
...
@register_model_architecture("polyconfm_property", "polyconfm_property")
