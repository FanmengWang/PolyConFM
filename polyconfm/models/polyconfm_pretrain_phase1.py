...
@register_model("polyconfm_pretrain_phase1")
class PolyConFMPretrainPhase1Model(BaseUnicoreModel):
    @staticmethod
    def add_args(parser):
    ...
    def __init__(self, args, dictionary):
        ...
        # --------------------------------------------------------------------------
        # 2D Encoder (within the Multi-modal Repeating Unit Encoder) basic specifics
        ...
        # --------------------------------------------------------------------------
        # 3D Encoder (within the Multi-modal Repeating Unit Encoder) basic specifics
        K = 128
    ...
    # Sampling the random order
    def sample_orders(self, bsz, seq_len, repeat_unit_actual_num):
        """Sample per-example generation orders over valid repeat-unit indices."""
        orders = []
    ...
    # Sampling the random mask based on the random order and the masking ratio
    def random_masking(self, x, orders, repeat_unit_actual_num):
        """Create MAR masks from sampled orders and a stochastic masking ratio."""
    ...
    # Encodering the inputs into the latents
    def forward_mae_encoder(self, x, mask, padding_mask, class_embedding, buffer_size):
        """Encode MAR tokens with class-conditioning buffer and rotary position encoding."""
        x = self.mar_z_proj(x)
    ...
    # Decodering the latents to predict the masked tokens
    def forward_mae_decoder(self, x, mask, padding_mask, buffer_size):
        """Decode MAR latents and restore masked token positions for diffusion heads."""
    ...
    def forward(
        ...
        **kwargs
        ...
        """Run phase-1 pretraining: repeat-unit encoding, MAR masking, and torsion score loss."""
        ...
        mar_orders = self.sample_orders(bsz=mar_x.size(0), seq_len=mar_x.shape[1], repeat_unit_actual_num=repeat_unit_actual_num) # (bsz, r_n)
        mar_mask = self.random_masking(mar_x, mar_orders, repeat_unit_actual_num) # (bsz, r_n)
        ...
        mar_x = self.forward_mae_encoder(mar_x, mar_mask, mar_padding_mask, mar_class_embedding, mar_buffer_size) # (bsz, mar_buffer_size + r_n, d)
        
        # MAR decoder
        mar_z = self.forward_mae_decoder(mar_x, mar_mask, mar_padding_mask, mar_buffer_size) # (bsz, r_n, d) 
        
        # MAR diffusion            
        assert (bsz * repeat_unit_num) == repeat_unit_torsion_pygs.num_graphs
        mask_mar_z = mar_z[(mar_mask.bool() & ~mar_padding_mask.bool())] # (bsz, r_n, d) -> (bsz * mar_mask_num, d)
        mask_repeat_unit_torsion_pygs = repeat_unit_torsion_pygs.to(src_tokens.device)[(mar_mask.bool() & ~mar_padding_mask.bool()).reshape(-1)] # (bsz, r_n) -> (bsz * mar_mask_num, )
        if self.args.mar_diff_batch_mul != 1:
            mask_mar_z = mask_mar_z.repeat(self.args.mar_diff_batch_mul, 1)
            mask_repeat_unit_torsion_pygs = mask_repeat_unit_torsion_pygs * self.args.mar_diff_batch_mul
        mask_repeat_unit_torsion_pygs_batch = Batch.from_data_list(mask_repeat_unit_torsion_pygs)  

        data = self.mar_diff_model(mask_repeat_unit_torsion_pygs_batch, mask_mar_z)
        pred = data.edge_pred
        score = torus.score(
...
# Positional embedding with rotary encoding
...
# Gaussian layer to encode the pairwise distance features into attention bias
...
@register_model_architecture("polyconfm_pretrain_phase1", "polyconfm_pretrain_phase1")
