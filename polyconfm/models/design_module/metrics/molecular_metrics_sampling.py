...
from polyconfm.models.design_module.metrics.property_metric import TaskModel
from polyconfm.models.design_module.analysis.rdkit_functions import compute_molecular_metrics
    ...
    def __init__(
        ...
        for cur_task in dataset_infos.task.split("-")[:]:
            ...
            model_path = os.path.join(
                dataset_infos.base_path, "evaluator", f"{cur_task}.joblib"
            )
            os.makedirs(os.path.dirname(model_path), exist_ok=True)
            evaluator = TaskModel(model_path, cur_task)
            self.task_evaluator[cur_task] = evaluator

    def forward(self, root_path, molecules, targets, name, current_epoch, val_counter, test=False):
        ...
        if test:
            file_name = os.path.join(root_path, "final_smiles.txt")
        ...
        result_to_csv(os.path.join(root_path, "output.csv"), all_logs)
        return all_smiles
