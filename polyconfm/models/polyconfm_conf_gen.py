...
from polyconfm.models.common_module.ipa_pytorch import IpaScore
from polyconfm.models.common_module.transformer_encoder_with_pair import TransformerEncoderWithPair

logger = logging.getLogger(__name__)

@register_model("polyconfm_conf_gen")
class PolyConFMConfGenModel(BaseUnicoreModel):
    @staticmethod
    def add_args(parser):
    ...
    def sample_orders(self, bsz, seq_len, repeat_unit_actual_num):
        """Sample autoregressive generation orders over valid repeat-unit slots."""
        orders = []
    ...
    def forward_mae_encoder(self, x, mask, padding_mask, class_embedding, buffer_size):
        """Encode MAR tokens with class-conditioning and buffer tokens."""
        x = self.mar_z_proj(x)
    ...
    def forward_mae_decoder(self, x, mask, padding_mask, buffer_size):
        """Decode MAR latent sequence and recover masked-token representations."""
    ...
    # Masking the input according to the sampled order
    def mask_by_order(self, mask_lens, order, bsz, seq_len):  
        """Create boolean masks by keeping the first mask_len indices in sampled order."""
    ...
    def embed_func(self, mol, numConfs):
        """Embed multiple initial conformers for an RDKit molecule."""
        AllChem.EmbedMultipleConfs(mol, numConfs=numConfs, numThreads=5)
        return mol


    # Samping the corresponding conformations of the current repeating unit
    def sample_confs(self, smi_idx, raw_smi, n_confs, smi, mar_z):
        """Sample and optionally refine conformers for one repeat unit."""
    ...
    def encoder_mar_diff_output(self, mol, device):
        """Encode a sampled molecule into repeat-unit latent features."""
    ...
    def forward(
        ...
        **kwargs
        ...
        """Autoregressively generate repeat units and run frame-diffusion refinement."""
        bsz = 1
    ...
    # Convert the rotation matrices to translation matrices.
    def rot2trans(self, rigids, input_coord, key_point_index, device):
        """Convert sampled rigid rotations into chained translations along polymer units."""
    ...
    def process_monomer(self, monomer_smiles):
        """Parse polymer-marked monomer SMILES and extract key-point atom indices."""
        smi = monomer_smiles
    ...
    def extract_confs(self, mols):
        """Collect 3D coordinates from generated molecule conformers."""
        confs_dict = {}
    ...
    def sample_init_feats(self, sample_length, input_coord, key_point_list, device):
        """Assemble initial frame-diffusion feature tensors for inference."""
    ...
    def inference(self, data_init, cond_info, num_t=None, min_t=None, center=True, self_condition=True, noise_scale=1.0, device=None):
        """Perform reverse-time frame diffusion to refine polymer rigid trajectories."""
    ...
    def set_t_feats(self, feats, t, t_placeholder):
        """Update timestep-dependent diffusion features in-place."""
...
# Positional embedding with sinusoidal encoding
...
# Positional embedding with sinusoidal encoding for time steps
...
# Embedding layer which combines time step embedding, sequence index embedding and MAR encoder output
...
@register_model_architecture("polyconfm_conf_gen", "polyconfm_conf_gen")
