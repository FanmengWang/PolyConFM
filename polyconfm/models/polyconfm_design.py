...
class PolyConFMDesignModel(BaseUnicoreModel):
    ...
    def forward_mae_encoder(self, x, mask, padding_mask, class_embedding, buffer_size):
        """Encode MAR tokens with class-conditioning buffer for design features."""
        x = self.mar_z_proj(x)
    ...
    def forward(
        ...
        **kwargs
        ...
        """Run design forward pass and return either training loss or sampled outputs."""
...
class DataInfos():
    def __init__(self, data_path, task_name, ensure_connected):
        """Load dataset metadata and distributions required by the design module."""
        tasktype_dict = {
            'O2': 'regression',
            'N2': 'regression',
            'CO2': 'regression',
...
def get_psmi_list(data_path, split_name):
    """Load and canonicalize polymer SMILES strings for a split."""
