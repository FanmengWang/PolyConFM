...
@register_model("polyconfm_pretrain_phase2")
class PolyConFMPretrainPhase2Model(BaseUnicoreModel):
    @staticmethod
    def add_args(parser):
    ...
    def forward_mae_encoder(self, x, mask, padding_mask, class_embedding, buffer_size):
        """Encode MAR inputs with class buffer for downstream frame diffusion conditioning."""
        x = self.mar_z_proj(x)
    ...
    def forward(
        ...
        **kwargs
        ...
        """Run phase-2 pretraining with frame-diffusion rotation and distance objectives."""
...
class Embedder(nn.Module):
    ...
    def __init__(self, model_conf, mar_encoder_embed_dim):
        """Initialize node/edge embedding stacks for frame score conditioning."""
    ...
    def _cross_concat(self, feats_1d, num_batch, num_res):
        """Construct pair features by concatenating tiled row/column token features."""
        return torch.cat([
    ...
    def forward(
        ...
        """Build node and edge embeddings from time, sequence index, and MAR features."""
...
@register_model_architecture("polyconfm_pretrain_phase2", "polyconfm_pretrain_phase2")
