...
from polyconfm.datas import (
    ...
    FrameDataset,
    RightPadDatasetRigid, 
    RightPadDatasetRigidScore
)
from unicore.tasks import UnicoreTask, register_task

logger = logging.getLogger(__name__)

@register_task("polyconfm_pretrain_phase2")
class PolyConFMPretrainPhase2Task(UnicoreTask):
    @staticmethod
    def add_args(parser):
        """Register task-specific CLI arguments.

        Args:
            parser: Argument parser to be extended with task options.
    ...
    def setup_task(cls, args, **kwargs):
        """Create phase-2 task instance after loading dictionary resources.

        Args:
            args: Parsed CLI/runtime arguments.
            **kwargs: Unused optional keyword arguments.

        Returns:
            Initialized PolyConFMPretrainPhase2Task instance.
        """
    ...
    def load_dataset(self, split, **kwargs):
        """Build phase-2 inputs including frame-diffusion supervision features."""
        ...
        key_point_index_dataset = KeyDataset(dataset, "key_point_index") # (bsz, 4)
        ...
        frame_dataset = FrameDataset(dataset)
        
        input_coord_dataset = KeyDataset(dataset, "repeat_unit_atom_coordinates")
        input_coord_dataset = FromNumpyDataset(input_coord_dataset)
        input_coord_dataset = PrependAndAppendCoord(input_coord_dataset, 0.0, 0.0) # (bsz, r_n, n, 3)
        
        tgt_coord_dataset = KeyDataset(dataset, "tgt_coordinates")
        tgt_coord_dataset = FromNumpyDataset(tgt_coord_dataset)
        tgt_coord_dataset = PrependAndAppendCoord(tgt_coord_dataset, 0.0, 0.0) # (bsz, r_n, n, 3)
        ...
        nest_dataset = NestedDictionaryDataset(
            {
                "net_input": {
                    ...
                    "key_point_index": key_point_index_dataset,
                    ...
                    "input_coord": RightPadDatasetCoord(
                        input_coord_dataset,
                        pad_idx=0,
                        pad_to_multiple=self.args.pad_to_multiple
                    ),
                    ...
                    "rigids_0": RightPadDatasetRigid(
                        KeyDataset(frame_dataset, 'rigids_0'),
                        pad_idx=0,
                        pad_to_multiple=self.args.pad_to_multiple
                    ),
                    "rigids_t": RightPadDatasetRigid(
                        KeyDataset(frame_dataset, 'rigids_t'),
                        pad_idx=0,
                        pad_to_multiple=self.args.pad_to_multiple
                    ),
                    "t": KeyDataset(frame_dataset, 't'),
                    "gt_trans_score": RightPadDatasetRigidScore(
                        KeyDataset(frame_dataset, 'trans_score'),
                        pad_idx=0,
                        pad_to_multiple=self.args.pad_to_multiple
                    ),
                    "gt_rot_score": RightPadDatasetRigidScore(
                        KeyDataset(frame_dataset, 'rot_score'),
                        pad_idx=0,
                        pad_to_multiple=self.args.pad_to_multiple
                    ),
                    "trans_score_scaling": KeyDataset(frame_dataset, 'trans_score_scaling'),
                    "rot_score_scaling": KeyDataset(frame_dataset, 'rot_score_scaling'),
                },
            },
        )
    ...
    def build_model(self, args):
        """Build the model associated with this task.

        Args:
            args: Parsed CLI/runtime arguments for model construction.

        Returns:
            Instantiated model object from the Unicore registry.
        """
