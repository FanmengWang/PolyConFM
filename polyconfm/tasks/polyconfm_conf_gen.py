...
@register_task("polyconfm_conf_gen")
class PolyConFMConfGenTask(UnicoreTask):
    @staticmethod
    def add_args(parser):
    ...
    def setup_task(cls, args, **kwargs):
        """Load dictionary resources and initialize the conformation-generation task."""
        dictionary = Dictionary.load(args.dict_name)
        logger.info("dictionary: {} types".format(len(dictionary)))
        return cls(args, dictionary)

    def load_dataset(self, split, **kwargs):
        """Build inference dataset fields required by the conformation model."""
    ...
    def build_model(self, args):
        """Construct the registered model instance for this task."""
