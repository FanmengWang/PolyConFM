...
def modify_conformer(pos, edge_index, mask_rotate, torsion_updates, as_numpy=False):
    """Apply torsional rotations to selected rotatable bonds.

    Args:
        pos: Atom coordinates as torch tensor or numpy array.
        edge_index: Rotatable bond list used for torsion updates.
        mask_rotate: Boolean mask indicating atoms affected by each bond.
        torsion_updates: Rotation angles (radians) for each rotatable bond.
        as_numpy: Whether to return numpy output instead of torch tensor.

    Returns:
        Updated coordinates with torsional perturbations applied.
    """
...
class TorsionNoiseTransform(BaseTransform):
    def __init__(self, sigma_min=0.01 * np.pi, sigma_max=np.pi):
        """Initialize torsional noise transform with sigma range.

        Args:
            sigma_min: Minimum torsion noise scale.
            sigma_max: Maximum torsion noise scale.

        Returns:
            None. Noise schedule bounds are stored.
        """
        self.sigma_min = sigma_min
        self.sigma_max = sigma_max

    def __call__(self, data):
        """Perturb one torsion graph sample and attach supervision fields.

        Args:
            data: PyG graph sample containing torsion topology metadata.

        Returns:
            Mutated graph sample with noisy coordinates and rotation labels.
        """
        try:
...
class ConformerDataset(BaseWrapperDataset):
    def __init__(self, dataset, repeat_unit_atom_symbols, repeat_unit_atom_coordinates, repeat_unit_torsion_pyg):
        """Wrap LMDB entries and attach torsion-noised repeat-unit conformers."""
    ...
    def set_epoch(self, epoch, **unused):
        """Set epoch value used by cached sampling logic.

        Args:
            epoch: Current training epoch identifier.
            **unused: Unused keyword arguments from caller.

        Returns:
            None. Internal epoch state is updated.
        """
    ...
    def __cached_item__(self, index: int, epoch: int):
        """Build and cache one sample with per-repeat-unit torsion graph data."""
    ...
    def __getitem__(self, index: int):
        """Fetch one cached sample by dataset index.

        Args:
            index: Integer index into the wrapped dataset.

        Returns:
            Dictionary containing repeat-unit symbols, coordinates, and PyGs.
        """
...
class PropertyConformerDataset(BaseWrapperDataset):
    def __init__(self, dataset, seed, split_name, repeat_unit_atom_symbols, repeat_unit_atom_coordinates):
        """Provide train/eval conformer sampling for property prediction."""
        self.dataset = dataset
        self.seed = seed
    ...
    def set_epoch(self, epoch, **unused):
        """Set epoch value used by deterministic train-time sampling.
    ...
    def __len__(self):
        """Return dataset length for train or evaluation mode.

        Args:
            None.

        Returns:
            Number of samples exposed for the current split.
        """
    ...
    @lru_cache(maxsize=16)
    def __cached_item__(self, index: int, epoch: int):
        """Return one sampled conformer and auxiliary polymer-level fields."""
        if self.split_name == 'train':
            size = len(self.dataset[0][self.repeat_unit_atom_coordinates])
    ...
    def __getitem__(self, index: int):
        """Fetch one sampled conformer entry.

        Args:
            index: Integer sample index in this wrapper dataset.

        Returns:
            Dictionary containing coordinates, labels, and polymer metadata.
        """
        return self.__cached_item__(index, self.epoch)
...
class DesignConformerDataset(BaseWrapperDataset):
    def __init__(self, dataset, seed, split_name, repeat_unit_atom_symbols, repeat_unit_atom_coordinates):
        """Provide train/eval conformer sampling for design optimization."""
        self.dataset = dataset
        self.seed = seed
        self.split_name = split_name
        self.repeat_unit_atom_symbols = repeat_unit_atom_symbols
    ...
    @lru_cache(maxsize=16)
    def __cached_item__(self, index: int, epoch: int):
        """Return one sampled conformer and design conditioning fields."""
        if self.split_name == 'train':
            size = len(self.dataset[0][self.repeat_unit_atom_coordinates])
            with data_utils.numpy_seed(self.seed, epoch, index):
                sample_idx = np.random.randint(size)
    ...
    def __getitem__(self, index: int):
        """Fetch one sampled design conformer entry.

        Args:
            index: Integer sample index in this wrapper dataset.

        Returns:
            Dictionary containing coordinates and design conditioning fields.
        """
        return self.__cached_item__(index, self.epoch)