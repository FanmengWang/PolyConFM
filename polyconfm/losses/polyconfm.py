...
class PolyConFMPretrainPhase1Loss(UnicoreLoss):
    ...
    def forward(self, model, sample, reduce=True):
        """Compute phase-1 training loss from model net_input fields."""
    ...
    def reduce_metrics(logging_outputs, split="valid") -> None:
        """Aggregate phase-1 loss metrics across workers."""
    ...
    def logging_outputs_can_be_summed(is_train) -> bool:
        """Indicate whether logging outputs can be summed across workers.

        Args:
            is_train: Whether current mode is training.

        Returns:
            Boolean flag used by the training framework for reduction.
        """
        return is_train
...
class PolyConFMPretrainPhase2Loss(UnicoreLoss):
    ...
    def forward(self, model, sample, reduce=True):
        """Compute phase-2 loss and report rotation and distance sub-losses."""
    ...
    @staticmethod
    def reduce_metrics(logging_outputs, split="valid") -> None:
        """Aggregate phase-2 total and auxiliary losses across workers."""
        loss_sum = sum(log.get("loss", 0) for log in logging_outputs)
...
class PolyConFMConfGenLoss(UnicoreLoss):
    ...
    def forward(self, model, sample, reduce=True):
        """Run conformation-generation model forward and pass through outputs."""
...
class PolyConFMPropertyLoss(UnicoreLoss):
    ...
    def forward(self, model, sample, reduce=True):
        """Compute standardized regression loss for property prediction."""
        net_output = model(
            **sample["net_input"],
            features_only=True,
    ...
    def compute_loss(self, model, net_output, sample, reduce=True):
        """Apply MSE on normalized targets for stable property training."""
    ...
    @staticmethod
    def reduce_metrics(logging_outputs, split="valid") -> None:
        """Aggregate property losses and report validation regression metrics."""
        loss_sum = sum(log.get("loss", 0) for log in logging_outputs)
        sample_size = sum(log.get("sample_size", 0) for log in logging_outputs)
        metrics.log_scalar(
            "loss", loss_sum / sample_size / math.log(2), sample_size, round=3
...
class PolyConFMDesignLoss(UnicoreLoss):
    ...
    def forward(self, model, sample, reduce=True):
        """Compute design-task diffusion loss from batched graph inputs."""
        loss = model(**sample["net_input"])
    ...
    @staticmethod
    def reduce_metrics(logging_outputs, split="valid") -> None:
        """Aggregate design-task loss across workers."""
        loss_sum = sum(log.get("loss", 0) for log in logging_outputs)
        sample_size = sum(log.get("sample_size", 0) for log in logging_outputs)
        metrics.log_scalar(
            "loss", loss_sum / sample_size / math.log(2), sample_size, round=3
...
class PolyConFMDesignInferenceLoss(UnicoreLoss):
    ...
    def forward(self, model, sample, reduce=True):
        """Run design inference forward and pass through generated outputs."""
        logging_output = model(**sample["net_input"])
        return 1, 1, logging_output