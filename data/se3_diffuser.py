...
def _extract_trans_rots(rigid: ru.Rigid):
    """Convert a Rigid object into numpy translation and rotation-vector arrays."""
...
def _assemble_rigid(rotvec, trans):
    """Assemble a Rigid object from rotation vectors and translations."""
...
class SE3Diffuser:
    ...
    def __init__(self, se3_conf):
        """Initialize coupled SO(3) and R^3 diffusers from configuration."""
    ...
    def calc_trans_0(self, trans_score, trans_t, t):
        """Estimate clean translations from noisy translations and score."""
    ...
    def calc_trans_score(self, trans_t, trans_0, t, use_torch=False, scale=True):
        """Compute translation score under the configured R^3 diffusion."""
        return self._r3_diffuser.score(
            trans_t, trans_0, t, use_torch=use_torch, scale=scale)

    def calc_rot_score(self, rots_t, rots_0, t):
        """Compute rotation score from noisy and clean rotations at time t."""
    ...
    def _apply_mask(self, x_diff, x_fixed, diff_mask):
        """Blend diffused and fixed values according to a diffusion mask."""
        return diff_mask * x_diff + (1 - diff_mask) * x_fixed

    def trans_parameters(self, trans_t, score_t, t, dt, mask):
        """Return reverse-step Gaussian parameters for translations."""
    ...
    def score(
            ...
            rigid_t: ru.Rigid,
            t: float):
        """Compute translation and rotation scores between clean and noisy rigids."""
    ...
    def score_scaling(self, t):
        """Return time-dependent score scaling factors for SO(3) and R^3."""
        rot_score_scaling = self._so3_diffuser.score_scaling(t)
        trans_score_scaling = self._r3_diffuser.score_scaling(t)
