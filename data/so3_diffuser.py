...
class SO3Diffuser:
    ...
    def __init__(self, so3_conf):
        """Initialize SO(3) diffusion cache tables and schedule parameters.

        Args:
            so3_conf: Configuration object with schedule and cache settings.

        Returns:
            None. Internal lookup tables and buffers are prepared in place.
        """
    ...
    def discrete_sigma(self):
        """Return discrete sigma grid used by cached IGSO(3) values.

        Args:
            None.

        Returns:
            1D numpy array of sigma values across diffusion time.
        """
        return self.sigma(
    ...
    def sigma_idx(self, sigma: np.ndarray):
        """Map sigma values to nearest discrete cache indices.

        Args:
            sigma: Sigma value(s) to project onto the discrete grid.

        Returns:
            Integer numpy array of cache indices.
        """
        return np.digitize(sigma, self.discrete_sigma) - 1

    def sigma(self, t: np.ndarray):
        """Evaluate schedule-defined sigma values at time t.

        Args:
            t: Diffusion time value(s) in [0, 1].

        Returns:
            Sigma value(s) from the configured schedule.
        """
    ...
    def diffusion_coef(self, t):
        """Compute diffusion coefficient g(t) for SO(3) reverse dynamics.

        Args:
            t: Diffusion time value(s) in [0, 1].

        Returns:
            Diffusion coefficient value(s) aligned with t.
        """
        if self.schedule == 'logarithmic':
            g_t = np.sqrt(
    ...
    def t_to_idx(self, t: np.ndarray):
        """Convert time values into discrete sigma-table indices.

        Args:
            t: Diffusion time value(s) in [0, 1].

        Returns:
            Integer index array into cached IGSO(3) tables.
        """
    ...
    def sample_ref(self, n_samples: float=1):
        """Sample rotations from the terminal-time reference distribution.

        Args:
            n_samples: Number of rotation vectors to generate.

        Returns:
            Array of axis-angle vectors with shape [n_samples, 3].
        """
    ...
    def torch_score(
        ...
        if self.use_cached_score:
            ...
            if omega_idx.dim() != score_norms_t.dim():
                omega_idx = omega_idx.unsqueeze(0)
    ...
    def score_scaling(self, t: np.ndarray):
        """Return cached score scaling factors for training targets.

        Args:
            t: Diffusion time value(s) in [0, 1].

        Returns:
            Scaling value(s) selected from the precomputed table.
        """
