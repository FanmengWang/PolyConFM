...
def try_mmff(mol):
    """Run MMFF optimization over current conformers.

    Args:
        mol: RDKit molecule containing one or more conformers.

    Returns:
        True if optimization runs successfully, otherwise False.
    """
    try:
...
def get_seed(smi, seed_confs=None):
    """Create an initial molecule-graph pair for sampling.

    Args:
        smi: Input SMILES string used for lookup or featurization.
        seed_confs: Optional mapping from SMILES to pre-generated conformers.

    Returns:
        Tuple of (mol, data) or (None, None) when initialization fails.
    """
    if seed_confs:
        ...
        mol, data = featurize_mol_from_smiles(smi)
        if not mol:
            return None, None
...
def embed_seeds(mol, data, n_confs, single_conf=False, smi=None, embed_func=None, seed_confs=None, pdb=None, mmff=False):
    """Build per-conformer inference inputs from embedded seed structures."""
    if not seed_confs:
...
def perturb_seeds(data, pdb=None):
    """Apply random torsion updates to seed conformers.

    Args:
        data: Iterable of conformer graph objects.
        pdb: Optional trajectory writer for visualization frames.

    Returns:
        The same conformer list with updated coordinates in place.
    """
...
def sample(conformers, model, sigma_max=np.pi, sigma_min=0.01 * np.pi, steps=20, batch_size=32,
           ...
           pg_invariant=False, mol=None, mar_z=None):
    """Run iterative torsion-space diffusion sampling for a conformer batch."""
...
def pyg_to_mol(mol, data, mmff=False, rmsd=True, copy=True):
    """Write graph coordinates back into an RDKit conformer object.

    Args:
        mol: RDKit molecule to update.
        data: Graph sample holding generated coordinates and metadata.
        mmff: Whether to run MMFF optimization after coordinate assignment.
        rmsd: Whether to compute RMSD against the seed molecule when possible.
        copy: Whether to return a deep copy of the molecule.

    Returns:
        Updated RDKit molecule containing the generated conformer.
    """
...
class InferenceDataset(Dataset):
    def __init__(self, data_list):
        """Build an inference dataset from conformer objects.

        Args:
            data_list: Sequence of PyG conformer data objects.
    ...
    def len(self):
        ...
        Returns:
            Number of conformer samples stored in the dataset.
        """
        return len(self.data)

    def get(self, idx):
        """Fetch one conformer sample.

        Args:
            idx: Integer dataset index.

        Returns:
            Conformer data object at the requested index.
        """
        return self.data[idx]

    def apply_torsion_and_update_pos(self, data, torsion_updates):
        """Apply torsion updates and persist positions into dataset entries.

        Args:
            data: Mini-batch of conformer graphs from the data loader.
            torsion_updates: Predicted torsion increments for that mini-batch.

        Returns:
            None. Coordinates and cumulative perturbations are updated in place.
        """
