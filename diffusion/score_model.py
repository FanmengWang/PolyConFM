...
class TensorProductConvLayer(torch.nn.Module):
    def __init__(self, in_irreps, sh_irreps, out_irreps, n_edge_features, residual=True, batch_norm=True):
        """Build one equivariant tensor-product message passing layer."""
    ...
    def forward(self, node_attr, edge_index, edge_attr, edge_sh, out_nodes=None, reduce='mean'):
        """Propagate edge-conditioned messages and aggregate to updated nodes."""
...
class TensorProductScoreModel(torch.nn.Module):
    def __init__(self, in_node_features=74, in_edge_features=4, sigma_embed_dim=32, sigma_min=0.01 * np.pi,
                 ...
                 scale_by_sigma=True, use_second_order_repr=True, batch_norm=True, residual=True, plus_hidden_dim=768
                 ):
        ...
        self.node_embedding = nn.Sequential(
            nn.Linear(in_node_features + sigma_embed_dim + plus_hidden_dim, ns),
            nn.ReLU(),
            nn.Linear(ns, ns)
        )
    ...
    def forward(self, data, mar_z):
        """Predict torsion score values for masked rotatable bonds."""
        node_attr, edge_index, edge_attr, edge_sh = self.build_conv_graph(data)
        src, dst = edge_index

        num_nodes_per_graph = torch.bincount(data['batch'])
        
        batch_mar_z = mar_z.repeat_interleave(num_nodes_per_graph, dim=0)
        processed_node_attr = torch.cat([node_attr, batch_mar_z], dim=1)
 
        node_attr = self.node_embedding(processed_node_attr)
    ...
    def build_bond_conv_graph(self, data, node_attr):
        """Construct bond-centered neighborhood graph for final scoring stage."""
    ...
    def build_conv_graph(self, data):
        """Create atom-level radius graph and associated equivariant features."""
...
class GaussianSmearing(torch.nn.Module):
    def __init__(self, start=0.0, stop=5.0, num_gaussians=50):
        """Initialize Gaussian radial basis parameters.

        Args:
            start: Minimum distance represented by the basis.
            stop: Maximum distance represented by the basis.
            num_gaussians: Number of Gaussian basis functions.

        Returns:
            None. Module buffers and coefficients are initialized in place.
        """
        super().__init__()
    ...
    def forward(self, dist):
        ...
        Args:
            dist: Distance tensor of arbitrary shape.

        Returns:
            Tensor with last dimension equal to num_gaussians.
        """
...
def get_timestep_embedding(timesteps, embedding_dim, max_positions=10000):
    ...
    Args:
        timesteps: 1D tensor of scalar timestep values.
        embedding_dim: Size of the output embedding dimension.
        max_positions: Frequency scaling constant for sinusoidal basis.

    Returns:
        Tensor of shape [num_timesteps, embedding_dim].
    """
